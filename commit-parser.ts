...
export function getDeploymentMessageTitle(event: DeploymentEvent): string {
  const message =
    event.eventType === "deployment_completed"
      ? event.commitMessage
      : event.prTitle;
...
function matchSingleServiceWithVersion(
  commitTitle: string,
): CommitMessageDetails | null {
  const pattern =
    /^Deploy\s+(\S+)\s+(\S+)\s+version\s+(v?\d+\.\d+\.\d+\S*)\s+to\s+(prod|sta|dev)(?:\s+\(#\d+\))?$/i;
...
function matchSingleServiceConfig(
  commitTitle: string,
): CommitMessageDetails | null {
  const pattern =
    /^Deploy\s+(\S+)\s+(\S+)\s+config\s+changes\s+to\s+(prod|sta|dev)(?:\s+\(#\d+\))?$/i;
  const match = commitTitle.match(pattern);
...
function matchMultipleServices(
  commitTitle: string,
): CommitMessageDetails | null {
  const pattern =
    /^Deploy\s+(\S+)\s+services\s+to\s+(prod|sta|dev)(?:\s+\(#\d+\))?$/i;
  const match = commitTitle.match(pattern);
...
function matchMultipleServicesConfig(
  commitTitle: string,
): CommitMessageDetails | null {
  const pattern =
    /^Deploy\s+(\S+)\s+services\s+config\s+changes\s+to\s+(prod|sta|dev)(?:\s+\(#\d+\))?$/i;
  const match = commitTitle.match(pattern);
...
function matchMultipleEnvironments(
  commitTitle: string,
): CommitMessageDetails | null {
  const pattern =
    /^Deploy\s+(\S+)\s+services\s+to\s+((?:sta|prod|dev)(?:\s+and\s+(?:sta|prod|dev))+)(?:\s+\(#\d+\))?$/i;
  const match = commitTitle.match(pattern);
...
function matchMultipleDomains(
  commitTitle: string,
): CommitMessageDetails | null {
  const pattern =
    /^Deploy\s+multiple\s+services\s+to\s+(prod|sta|dev)(?:\s+\(#\d+\))?$/i;
  const match = commitTitle.match(pattern);
...
function matchMultipleDomainsConfig(
  commitTitle: string,
): CommitMessageDetails | null {
  const pattern =
    /^Deploy\s+config\s+changes\s+to\s+(prod|sta|dev)(?:\s+\(#\d+\))?$/i;
  const match = commitTitle.match(pattern);
...
function matchLegacyV1Format(commitTitle: string): CommitMessageDetails | null {
  const pattern =
    /^Deployed\s+(\S+)\s+(\S+)\s+version\s+(v?\d+\.\d+\.\d+\S*)\s+to\s+(prod|sta|dev)(?:\s+\(#\d+\))?$/i;
  const match = commitTitle.match(pattern);
