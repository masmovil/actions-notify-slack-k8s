...
describe('actions-notify-slack-k8s', () => {
  ...
  it('should parse different deployment environments', () => {
    const testCases = [
      {
        ...
        expected: { domain: 'mas-billing', service: 'api-billing', version: 'v1.37.0', environment: 'prod' }
      },
      {
        message: 'Deployed mas-billing api-billing version v1.37.0-RC.2 to sta',
        expected: { domain: 'mas-billing', service: 'api-billing', version: 'v1.37.0-RC.2', environment: 'sta' }
      },
      {
        message: 'Deployed mas-billing api-billing version v1.37.0 to DEV',
        expected: { domain: 'mas-billing', service: 'api-billing', version: 'v1.37.0', environment: 'DEV' }
      }
    ]
    ...
    testCases.forEach(testCase => {
      ...
      expect(result.commitMessage.environment).toBe(testCase.expected.environment)
    })
  })
    ...
    const testCases = [
      {
        ...
        expected: { domain: 'mas-billing', service: 'rating-engine', version: 'v1.132.5', environment: 'prod' }
      },
      {
        message: 'Deploy mas-billing api-billing version v2.0.0-RC.1 to sta',
        expected: { domain: 'mas-billing', service: 'api-billing', version: 'v2.0.0-RC.1', environment: 'sta' }
      },
      {
        message: 'Deploy mas-billing payment-service version v3.1.0 to dev',
        expected: { domain: 'mas-billing', service: 'payment-service', version: 'v3.1.0', environment: 'dev' }
      }
    ]
